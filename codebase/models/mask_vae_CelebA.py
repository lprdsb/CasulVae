...
class CausalVAE(nn.Module):
    ...
    def negative_elbo_bound(self, x, label = None, mask = None, sample = False, adj = None, alpha=0.3, beta=1, lambdav=0.001):
        """
        ...
        """
        assert label == None or label.size()[1] == self.z1_dim
        if label == None: label = torch.ones(x.shape[0], self.z1_dim, dtype = x.dtype, device = x.device)
        ...
        decoded_bernoulli_logits,x1,x2,x3,x4 = self.dec.decode_sep(z_given_dag.reshape([z_given_dag.size()[0], self.z_dim]), None)
        # print(decoded_bernoulli_logits.shape)
        ...
        # print(cp_m.shape, cp_v.shape, decode_m.shape)
